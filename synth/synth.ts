...
export class Song {
    ...
    private static readonly _oldestLunariiBoxVersion: number = 1;
    private static readonly _latestLunariiBoxVersion: number = 1;
    // One-character variant detection at the start of URL to distinguish variants such as JummBox, Or Goldbox. "j" and "g" respectively
    //also "u" is ultrabox lol
    private static readonly _variant = 0x6C; //"l" ~ lunariis box
    ...
    public toBase64String(): string {
        ...
        buffer.push(base64IntToCharCode[Song._latestLunariiBoxVersion]);
    ...
    public fromBase64String(compressed: string, jsonFormat: string = "auto"): void {
        ...
        let fromLunariisBox: boolean = false;
        ...
        if (variantTest == 0x6A) { //"j"
            ...
            charIndex++;
        } else if(variantTest == 0x6C){ //"l"
                fromLunariisBox = true
                charIndex++;
        } else {
            fromBeepBox = true;
        }
        ...
        if (fromLunariisBox && (version == -1 || version > Song._latestLunariiBoxVersion || version < Song._oldestLunariiBoxVersion)) return;
        ...
        if (fromSlarmoosBox || fromUltraBox || fromGoldBox || fromLunariisBox) {
        ...
        while (charIndex < compressed.length) switch (command = compressed.charCodeAt(charIndex++)) {
            ...
            case SongTagCode.rhythm: {
                if (!fromUltraBox && !fromSlarmoosBox && !fromLunariisBox) {
            ...
            case SongTagCode.wave: {
                if (beforeThree && fromBeepBox) {
                    ...
                    if (this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].type == InstrumentType.noise) {
                        ...
                        if (fromSlarmoosBox || fromUltraBox || fromLunariisBox) {
            ...
            case SongTagCode.eqFilter: {
                if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
                    ...
                    if (fromBeepBox || typeCheck == 0) {
                        ...
                        if (fromJummBox || fromGoldBox || fromUltraBox || fromSlarmoosBox || fromLunariisBox)
                        ...
                        if ((fromJummBox && !beforeFive) || (fromGoldBox && !beforeFour) || fromUltraBox || fromSlarmoosBox || fromLunariisBox) {
            ...
            case SongTagCode.drumsetEnvelopes: {
                ...
                const pregoldToEnvelope: number[] = [0, 1, 2, 4, 5, 6, 8, 9, 10, 12, 13, 14, 16, 17, 18, 19, 20, 21, 23, 24, 25, 27, 28, 29, 32, 33, 34, 31, 11];
                if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
                    if ((beforeTwo && fromGoldBox) || (!fromGoldBox && !fromUltraBox && !fromSlarmoosBox && !fromLunariisBox)) {

                    }
                    if (instrument.type == InstrumentType.drumset) {
                        for (let i: number = 0; i < Config.drumCount; i++) {
                            let aa: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                            if ((beforeTwo && fromGoldBox) || (!fromGoldBox && !fromUltraBox && !fromSlarmoosBox && !fromLunariisBox)) aa = pregoldToEnvelope[aa];
                    ...
                    for (let i: number = 0; i < Config.drumCount; i++) {
                        let aa: number = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                        if ((beforeTwo && fromGoldBox) || (!fromGoldBox && !fromUltraBox && !fromSlarmoosBox && !fromLunariisBox)) aa = pregoldToEnvelope[aa];
                        if (!fromSlarmoosBox && !fromLunariisBox && aa >= 2) aa++; //2 for pitch
            ...
            case SongTagCode.pulseWidth: {
                ...
                if ((fromUltraBox && !beforeFour) || fromSlarmoosBox || fromLunariisBox) {
            ...
            case SongTagCode.fadeInOut: {
                if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
                    ...
                    } else if ((beforeFour && !fromGoldBox && !fromUltraBox && !fromSlarmoosBox && !fromLunariisBox) || fromBeepBox) {
                        const settings = legacySettings[clamp(0, legacySettings.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)])];
                        const instrument: Instrument = this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
                        instrument.fadeIn = Synth.secondsToFadeInSetting(settings.fadeInSeconds);
                        instrument.fadeOut = Synth.ticksToFadeOutSetting(settings.fadeOutTicks);
            ...
            case SongTagCode.songEq: { //deprecated vibrato tag repurposed for songEq
                if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
                    ...
                    // songeq
                    if ((fromSlarmoosBox && !beforeFour) || (fromLunariisBox)){ //double check that it's from a valid version
            ...
            case SongTagCode.unison: {
                if (beforeThree && fromBeepBox) {
                    ...
                    const unisonLength = (beforeFive || !fromSlarmoosBox || !fromLunariisBox) ? 27 : Config.unisons.length; //27 was the old length before I added >2 voice presets
                ...
                if ((beforeNine && fromBeepBox) || ((fromJummBox && beforeFive) || (beforeFour && fromGoldBox))) {
                    ...
                    if (effectsIncludeNoteFilter(instrument.effects)) {
                        ...
                        if (fromBeepBox || typeCheck == 0) {
                            ...
                            if ((fromJummBox && !beforeFive) || (fromGoldBox) || (fromUltraBox) || (fromSlarmoosBox) || (fromLunariisBox)) {
                                let usingSubFilterBitfield: number = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6) | (base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                                for (let j: number = 0; j < Config.filterMorphCount - 1; j++) {
                                    if (usingSubFilterBitfield & (1 << j)) {
                                        // Number of control points
                    ...
                    if (effectsIncludeChord(instrument.effects)) {
                        ...
                        if (instrument.chord == Config.chords.dictionary["arpeggio"].index && (fromJummBox || fromGoldBox || fromUltraBox || fromSlarmoosBox || fromLunariisBox)) {
                    ...
                    if (effectsIncludeVibrato(instrument.effects)) {
                        ...
                        // Custom vibrato
                        if (instrument.vibrato == Config.vibratos.length && (fromJummBox || fromGoldBox || fromUltraBox || fromSlarmoosBox || fromLunariisBox)) {
                    ...
                    if (effectsIncludePanning(instrument.effects)) {
                        ...
                        if ((fromJummBox && !beforeTwo) || fromGoldBox || fromUltraBox || fromSlarmoosBox || fromLunariisBox)
            ...
            case SongTagCode.pan: {
                if (beforeNine && fromBeepBox) {
                    ...
                    if (fromJummBox && !beforeThree || fromGoldBox || fromUltraBox || fromSlarmoosBox || fromLunariisBox) {
                        instrument.panDelay = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                    }
                } else {
                    // Do nothing? This song tag code is deprecated for now.
            ...
            case SongTagCode.channelNames: {
                for (let channel: number = 0; channel < this.getChannelCount(); channel++) {
                    ...
                    if (beforeFour && !fromGoldBox && !fromUltraBox && !fromSlarmoosBox && !fromLunariisBox)
            ...
            case SongTagCode.operatorFrequencies: {
                ...
                else if (!fromGoldBox && !fromUltraBox && !fromSlarmoosBox && !fromLunariisBox) {
            ...
            case SongTagCode.envelopes: {
                ...
                if ((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox)) {
                    ...
                    for (let i: number = 0; i < envelopeCount; i++) {
                        ...
                        let isTremolo2: boolean = false;
                        if ((fromSlarmoosBox && !beforeThree && beforeFour) || (fromLunariisBox) || updatedEnvelopes) { //remove tremolo2
                            if(aa == 9) isTremolo2 = true;
                            aa = slarURL3toURL4Envelope[aa]; 
                        }
                        const envelope: number = clamp(0, (((fromSlarmoosBox && !beforeThree) || (fromLunariisBox) || updatedEnvelopes) ? Config.newEnvelopes.length : Config.envelopes.length), aa);
                        let pitchEnvelopeStart: number = 0;
                        let pitchEnvelopeEnd: number = Config.maxPitch;
                        let envelopeInverse: boolean = false;
                        perEnvelopeSpeed = ((fromSlarmoosBox && !beforeThree) || (fromLunariisBox)) ? Config.newEnvelopes[envelope].speed : perEnvelopeSpeed;
                        ...
                        if ((fromSlarmoosBox && !beforeFour) || (fromLunariisBox)) {
                        ...
                        if ((fromSlarmoosBox && !beforeThree) || (fromLunariisBox) || updatedEnvelopes) {
                            ...
                            if (!useOldShape && !newNote) {
                                ...
                                if (note.start == 0) {
                                    if (!((beforeNine && fromBeepBox) || (beforeFive && fromJummBox) || (beforeFour && fromGoldBox))) {
                                        ...
                                        if ((beforeFour && !fromUltraBox && !fromSlarmoosBox && !fromLunariisBox ) || fromBeepBox) {
                                            note.continuesLastPattern = false;
                                        } else {
...
private static analogSynth(
        ...
        // Define waveforms
        const waveforms: { [key: string]: (phase: number) => number } = {
        ...
        type WaveformName = keyof typeof waveforms;

        // Oscillator parameters
        // Use instrumentState.carrierWaveform and instrumentState.modulatorWaveform if set, otherwise default to "sine".
        // These should be settable from the instrument editor UI.
        let carrierWaveform: WaveformName = "sine";
        let modulatorWaveform: WaveformName = "sine";
        if ((instrumentState as any).carrierWaveform) carrierWaveform = (instrumentState as any).carrierWaveform;
        if ((instrumentState as any).modulatorWaveform) modulatorWaveform = (instrumentState as any).modulatorWaveform;

        const modDepth: number = (instrumentState as any).modDepth ?? 0; // 0 = no mod, 1 = full mod
        const modRate: number = (instrumentState as any).modRate ?? 1; // multiplier for modulator phase delta

        // Carrier and modulator phase
        let carrierPhase: number = tone.phases[0];
        let modulatorPhase: number = tone.phases[1];

        const carrierPhaseDelta: number = tone.phaseDeltas[0];
        const modulatorPhaseDelta: number = (tone.phaseDeltas[1] ?? tone.phaseDeltas[0]) * modRate;

        const carrierFn = waveforms[carrierWaveform] || waveforms.sine;
        const modulatorFn = waveforms[modulatorWaveform] || waveforms.sine;

        for (let sampleIndex = bufferIndex; sampleIndex < bufferIndex + roundedSamplesPerTick; sampleIndex++) {
            // Modulator output in [-1, 1]
            const modValue = modulatorFn(modulatorPhase % 1);

            // Amplitude modulation: carrier amplitude is affected by modulator
            // modDepth: 0 = no mod, 1 = full mod (amplitude varies from 0 to 2)
            const amplitude = 1 + modValue * modDepth;

            const carrierSample = carrierFn(carrierPhase % 1) * amplitude;

            data[sampleIndex] += carrierSample * tone.expression;
            tone.expression += tone.expressionDelta;

            carrierPhase += carrierPhaseDelta;
            modulatorPhase += modulatorPhaseDelta;
        }

        // Write back phases
        tone.phases[0] = carrierPhase;
        tone.phases[1] = modulatorPhase;
}
    // advloop addition
